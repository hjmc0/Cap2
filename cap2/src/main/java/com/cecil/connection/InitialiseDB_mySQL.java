...
public class InitialiseDB_mySQL {
    ...
    public static void createDB() {
        String createAcc = "CREATE TABLE IF NOT EXISTS Account (aid INT PRIMARY KEY, aname VARCHAR(50) NOT NULL, email VARCHAR(30), phone INT(12), address VARCHAR(30), balance DECIMAL(38,2) NOT NULL);";
        String createTrans = "CREATE TABLE IF NOT EXISTS Transaction (trans_id INT PRIMARY KEY, trans_date TIMESTAMP NOT NULL, trans_type VARCHAR(10) NOT NULL, aid INT, FOREIGN KEY (aid) REFERENCES Account(aid) ON DELETE CASCADE, amount DECIMAL(38,2) NOT NULL);";
        String createTeller = "CREATE TABLE IF NOT EXISTS teller (tname VARCHAR(6), tpass VARCHAR(6));";
        try {
